/**
 * E2E Tests for Avalanche Chain Swap Flow
 * 
 * Tests complete user journey for swapping on Avalanche (Chain ID: 43114)
 */
...
describe('Avalanche Chain E2E Swap Flow', () => {
    ...
    beforeEach(() => {
        ...
        useChainId.mockReturnValue(AVALANCHE_CHAIN_ID);
        ...
        chainConfig.getChain.mockReturnValue({
            chainId: AVALANCHE_CHAIN_ID.toString(),
            chainName: 'Avalanche',
            type: 'EVM',
            blockExplorers: [{ url: 'https://snowtrace.io' }],
        });

        chainConfig.getContractAddress.mockImplementation((chainId, type) => {
            if (chainId === AVALANCHE_CHAIN_ID) {
                return { router: '0xAvalancheRouter', referral: '0xAvalancheReferral', token: '0xAvalancheToken' }[type] || null;
            }
            return null;
        });

        chainConfig.getExplorerUrl.mockReturnValue(`https://snowtrace.io/tx/${MOCK_TX_HASH}`);
    ...
    describe('Network Switching', () => {
        test('should switch to Avalanche network', async () => {
    ...
    describe('Token Selection', () => {
        test('should display Avalanche token list', async () => {
            render(<SwapBox />);
            expect(screen.getByTestId("swap-box")).toBeInTheDocument();
        });
    });
    ...
    describe('Swap Execution', () => {
        test('should execute swap on Avalanche', async () => {
            render(<SwapBox />);
    ...
    describe('Transaction Tracking', () => {
        test('should track Avalanche transaction', async () => {
            render(<SwapBox />);
            expect(screen.getByTestId("swap-box")).toBeInTheDocument();
        });
    });
    ...
    describe('Explorer Links', () => {
        test('should generate Avalanche explorer URL', () => {
            render(<SwapBox />);
            const AVALANCHE_CHAIN_ID = 43114;
            const explorerUrl = chainConfig.getExplorerUrl(AVALANCHE_CHAIN_ID, MOCK_TX_HASH);
            expect(explorerUrl).toBeDefined();
            expect(chainConfig.getExplorerUrl).toHaveBeenCalledWith(AVALANCHE_CHAIN_ID, MOCK_TX_HASH);
        });
    });
});
