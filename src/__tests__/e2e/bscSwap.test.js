/**
 * E2E Tests for BSC Chain Swap Flow
 * 
 * Tests complete user journey for swapping on BSC (Chain ID: 56)
 */
...
describe('BSC Chain E2E Swap Flow', () => {
    ...
    beforeEach(() => {
        ...
        useChainId.mockReturnValue(BSC_CHAIN_ID);
        ...
        chainConfig.getContractAddress.mockImplementation((chainId, type) => {
            if (chainId === BSC_CHAIN_ID) {
                return { router: '0xBSCRouter', referral: '0xBSCReferral', token: '0xBSCToken' }[type] || null;
            }
            return null;
        });

        chainConfig.getExplorerUrl.mockReturnValue(`https://bscscan.com/tx/${MOCK_TX_HASH}`);
    ...
    describe('Token Selection', () => {
        test('should display BSC token list', async () => {
            render(<SwapBox />);
            expect(screen.getByTestId("swap-box")).toBeInTheDocument();
        });
    });
    ...
    describe('Swap Execution', () => {
        test('should execute swap on BSC', async () => {
            render(<SwapBox />);
    ...
    describe('Transaction Tracking', () => {
        test('should track BSC transaction', async () => {
            render(<SwapBox />);
            expect(screen.getByTestId("swap-box")).toBeInTheDocument();
        });
    });
    ...
    describe('Explorer Links', () => {
        test('should generate BSC explorer URL', () => {
            render(<SwapBox />);
            const BSC_CHAIN_ID = 56;
            const explorerUrl = chainConfig.getExplorerUrl(BSC_CHAIN_ID, MOCK_TX_HASH);
            expect(explorerUrl).toBeDefined();
            expect(chainConfig.getExplorerUrl).toHaveBeenCalledWith(BSC_CHAIN_ID, MOCK_TX_HASH);
        });
    });
});
