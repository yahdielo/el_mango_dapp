/**
 * E2E Tests for Polygon Chain Swap Flow
 * 
 * Tests complete user journey for swapping on Polygon (Chain ID: 137)
 */
...
describe('Polygon Chain E2E Swap Flow', () => {
    ...
    beforeEach(() => {
        ...
        useChainId.mockReturnValue(POLYGON_CHAIN_ID);
        ...
        chainConfig.getChain.mockReturnValue({
            ...
            chainName: 'Polygon',
            type: 'EVM',
            blockExplorers: [{ url: 'https://polygonscan.com' }],
        });

        chainConfig.getContractAddress.mockImplementation((chainId, type) => {
            if (chainId === POLYGON_CHAIN_ID) {
                return { router: '0xPolygonRouter', referral: '0xPolygonReferral', token: '0xPolygonToken' }[type] || null;
            }
            return null;
        });

        chainConfig.getExplorerUrl.mockReturnValue(`https://polygonscan.com/tx/${MOCK_TX_HASH}`);
    ...
    describe('Network Switching', () => {
        test('should switch to Polygon network', async () => {
    ...
    describe('Token Selection', () => {
        test('should display Polygon token list', async () => {
            render(<SwapBox />);
            expect(screen.getByTestId("swap-box")).toBeInTheDocument();
        });
    });
    ...
    describe('Swap Execution', () => {
        test('should execute swap on Polygon', async () => {
            render(<SwapBox />);
    ...
    describe('Transaction Tracking', () => {
        test('should track Polygon transaction', async () => {
            render(<SwapBox />);
            expect(screen.getByTestId("swap-box")).toBeInTheDocument();
        });
    });
    ...
    describe('Explorer Links', () => {
        test('should generate Polygon explorer URL', () => {
            const explorerUrl = chainConfig.getExplorerUrl(POLYGON_CHAIN_ID, MOCK_TX_HASH);
            expect(explorerUrl).toBeDefined();
            expect(chainConfig.getExplorerUrl).toHaveBeenCalledWith(POLYGON_CHAIN_ID, MOCK_TX_HASH);
        });
    });
});
